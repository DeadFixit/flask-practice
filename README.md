...
#### Если при пуше появляется сообщение «Бинарные файлы не поддерживаются»
- Убедитесь, что в коммите нет файла `instance/citygreenhub.sqlite` или других артефактов из папки `instance/` — они генерируются локально и исключены через `.gitignore`.
- Если файл уже был проиндексирован, удалите его из индекса и снова закоммитьте:
  ```bash
  git rm --cached instance/citygreenhub.sqlite
  git commit -m "Remove generated SQLite database"
  ```
  После этого push пройдёт без бинарных файлов.
